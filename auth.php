 ...
 */
class auth_plugin_dbparent extends auth_plugin_base {
    ...
    function __construct() {
        ...
        $this->authtype = 'dbparent';
        $this->config = get_config('auth/dbparent');
    ...
    function user_login($username, $password) {
        ...
        if ($this->is_internal()) {
            ...
            if (!$rs) {
                $authdb->Close();
                debugging(get_string('auth_dbcantconnect','auth_dbparent'));
                return false;
            }
    ...
    function sync_users(progress_trace $trace, $do_updates=false) {
        ...
        if (!empty($this->config->removeuser)) {
            ...
            if (!empty($remove_users)) {
                ...
                $trace->output(get_string('auth_dbuserstoremove','auth_dbparent', count($remove_users)));

                foreach ($remove_users as $user) {
                    if ($this->config->removeuser == AUTH_REMOVEUSER_FULLDELETE) {
                        delete_user($user);
                        $trace->output(get_string('auth_dbdeleteuser', 'auth_dbparent', array('name'=>$user->username, 'id'=>$user->id)), 1);
                    } else if ($this->config->removeuser == AUTH_REMOVEUSER_SUSPEND) {
                        $updateuser = new stdClass();
                        $updateuser->id   = $user->id;
                        $updateuser->suspended = 1;
                        user_update_user($updateuser, false);
                        $trace->output(get_string('auth_dbsuspenduser', 'auth_dbparent', array('name'=>$user->username, 'id'=>$user->id)), 1);
                    }
                }
            }
            unset($remove_users);
                ...
                if ($update_users = $DB->get_records_sql($sql, $params)) {
                    ...
                    foreach ($update_users as $user) {
                        if ($this->update_user_record($user->username, $updatekeys)) {
                            $trace->output(get_string('auth_dbupdatinguser', 'auth_dbparent', array('name'=>$user->username, 'id'=>$user->id)), 1);
                        } else {
                            $trace->output(get_string('auth_dbupdatinguser', 'auth_dbparent', array('name'=>$user->username, 'id'=>$user->id))." - ".get_string('skipped'), 1);
                        }
                    }
                    unset($update_users);
                }
        ...
        if (!empty($add_users)) {
            $trace->output(get_string('auth_dbuserstoadd','auth_dbparent',count($add_users)));
            ...
            foreach($add_users as $user) {
                ...
                if ($this->config->removeuser == AUTH_REMOVEUSER_SUSPEND) {
                    if ($old_user = $DB->get_record('user', array('username'=>$username, 'deleted'=>0, 'suspended'=>1, 'mnethostid'=>$CFG->mnet_localhost_id, 'auth'=>$this->authtype))) {
                        ...
                        $trace->output(get_string('auth_dbreviveduser', 'auth_dbparent', array('name'=>$username, 'id'=>$old_user->id)), 1);
                        continue;
                    }
                }
                ...
                if ($collision = $DB->get_record_select('user', "username = :username AND mnethostid = :mnethostid AND auth <> :auth", array('username'=>$user->username, 'mnethostid'=>$CFG->mnet_localhost_id, 'auth'=>$this->authtype), 'id,username,auth')) {
                    $trace->output(get_string('auth_dbinsertuserduplicate', 'auth_dbparent', array('username'=>$user->username, 'auth'=>$collision->auth)), 1);
                    continue;
                }
                try {
                    $id = $DB->insert_record ('user', $user); // it is truly a new user
                    $trace->output(get_string('auth_dbinsertuser', 'auth_dbparent', array('name'=>$user->username, 'id'=>$id)), 1);
                } catch (moodle_exception $e) {
                    $trace->output(get_string('auth_dbinsertusererror', 'auth_dbparent', $user->username), 1);
                    continue;
                }
    ...
    function user_exists($username) {
        ...
        if (!$rs) {
            print_error('auth_dbcantconnect','auth_dbparent');
        } else if (!$rs->EOF) {
    ...
    function update_user_record($username, $updatekeys=false) {
        ...
        if (empty($user)) { // trouble
            ...
            print_error('auth_dbusernotexist','auth_dbparent',$username);
            die;
        }
    ...
    function process_config($config) {
        ...
        // Save settings.
        set_config('host',          $config->host,          'auth/dbparent');
        set_config('type',          $config->type,          'auth/dbparent');
        set_config('sybasequoting', $config->sybasequoting, 'auth/dbparent');
        set_config('name',          $config->name,          'auth/dbparent');
        set_config('user',          $config->user,          'auth/dbparent');
        set_config('pass',          $config->pass,          'auth/dbparent');
        set_config('table',         $config->table,         'auth/dbparent');
        set_config('fielduser',     $config->fielduser,     'auth/dbparent');
        set_config('fieldpass',     $config->fieldpass,     'auth/dbparent');
        set_config('passtype',      $config->passtype,      'auth/dbparent');
        set_config('extencoding',   trim($config->extencoding), 'auth/dbparent');
        set_config('setupsql',      trim($config->setupsql),'auth/dbparent');
        set_config('debugauthdb',   $config->debugauthdb,   'auth/dbparent');
        set_config('removeuser',    $config->removeuser,    'auth/dbparent');
        set_config('changepasswordurl', trim($config->changepasswordurl), 'auth/dbparent');
